...
function renderList(data){
    ...
    data.forEach((item)=>{
        ...
        if(item.type){
            ...
            type.style.setProperty("--tag-rest-tilt", (4 + Math.random() * 4).toFixed(1) + "deg");
        }
        ...
        if(regionName){
            ...
            regionTag.style.setProperty("--tag-rest-tilt", (4 + Math.random() * 4).toFixed(1) + "deg");
        }
...
// 關閉按鈕：直接關閉彈窗，開始下滑動畫。
// （原本這裡會先額外播一次「液態玻璃」點擊回饋動畫、播完才關閉，
//  但這段回饋跟按下/放開時 :active 的縮放回饋重複，感覺像多播了一次動畫，
//  且會讓面板多卡一段時間才開始下滑，所以拿掉，改成按下就立即關閉。）
function closeModal(){
    finishCloseModal();
}
